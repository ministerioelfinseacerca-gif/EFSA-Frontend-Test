      ...
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-2 flex flex-row flex-wrap md:flex-nowrap items-center justify-between gap-y-2 gap-x-4">
        ...
        <div className="flex items-center gap-4 md:gap-6 justify-center">
          ...
          <motion.button
            ...
            className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gold-base text-navy-midnight border-2 border-gold-base hover:bg-gold-highlight hover:border-gold-highlight flex items-center justify-center cursor-pointer"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            aria-label={isPlaying ? 'Pausar Radio' : 'Reproducir Radio'}
          >
            {isPlaying ? (
              <Pause className="h-5 w-5 md:h-6 md:w-6 text-navy-midnight fill-current" />
            ) : (
              <Play className="h-5 w-5 md:h-6 md:w-6 text-navy-midnight fill-current ml-0.5" />
            )}
          </motion.button>
