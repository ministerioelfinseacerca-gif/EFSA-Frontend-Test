  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="mb-8 flex justify-center">
        <div className="inline-flex bg-white border border-navy-midnight/10 p-1 rounded-full shadow-sm">
          <button
            type="button"
            onClick={() => setMode('audio')}
            className={`flex items-center gap-2 px-6 py-3 text-xs font-black uppercase tracking-widest transition-all duration-300 cursor-pointer rounded-full ${
              mode === 'audio'
                ? 'bg-navy-midnight text-white shadow-md'
                : 'bg-transparent text-navy-midnight/60 hover:text-navy-midnight hover:bg-navy-midnight/5'
            }`}
          >
...
function AudioLivePlayer() {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
        ...
        {RADIO_STATIONS.map((s) => {
          ...
          return (
            <button
              ...
              className={`group flex items-center justify-between border p-4 text-left transition-all duration-300 cursor-pointer rounded-2xl ${
                active
                  ? 'border-navy-midnight bg-navy-midnight text-white shadow-md'
                  : 'border-navy-midnight/10 bg-white hover:border-navy-midnight/30 text-navy-midnight'
              }`}
            >
              <div className="flex items-center gap-3 min-w-0">
                <span
                  className={`flex h-12 w-12 rounded-full shrink-0 items-center justify-center font-black text-sm border ${
                    active ? 'border-navy-midnight bg-white text-navy-midnight' : 'border-navy-midnight/10 bg-editorial-beige text-navy-midnight/70'
                  }`}
                >
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-light text-xs opacity-60 truncate mt-1">{s.city}</p>
                </div>
              </div>
            </button>
          );
      ...
      {/* Main player box */}
      <div className="relative overflow-hidden border border-navy-midnight/10 bg-white rounded-3xl p-6 sm:p-8 shadow-sm mt-4">
        ...
        <div className="relative flex flex-col gap-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`font-black text-sm uppercase tracking-tight ${isLive ? 'text-sky-800' : 'text-navy-midnight/70'}`}>
                ...
                <p className="font-light text-xs text-navy-midnight/60 mt-1">
          ...
          <div className="flex flex-col items-center text-center mt-2 mb-2">
            <h3 className="font-black text-3xl sm:text-4xl text-navy-midnight leading-[1.05] tracking-tighter mb-1">
              Radio <br className="hidden sm:block" />
              <span className="font-light italic text-sky-800 tracking-tight">Tiempos Finales.</span>
            </h3>
            <span className="font-light text-sm text-sky-800 mt-1">
              {station.frequency}
            </span>
          </div>
          ...
          <div className="flex items-center justify-center">
            <motion.button
              ...
              className="relative flex h-24 w-24 items-center justify-center rounded-full border-2 border-navy-midnight/20 text-navy-midnight bg-transparent hover:border-navy-midnight hover:bg-navy-midnight hover:text-white transition-all duration-300 disabled:opacity-80 cursor-pointer shadow-sm"
            >
              {isLive && (
                <span className="absolute inset-0 rounded-full border-2 border-navy-midnight opacity-20 animate-ping"></span>
              )}
          ...
          <div className="flex items-center gap-3 bg-white px-6 py-4 border border-navy-midnight/10 rounded-full shadow-sm max-w-sm mx-auto w-full">
            <button
              type="button"
              onClick={() => setMuted((m) => !m)}
              aria-label={muted ? 'Activar sonido' : 'Silenciar'}
              className="h-10 w-10 shrink-0 flex items-center justify-center rounded-full border-2 border-navy-midnight/10 text-navy-midnight hover:border-navy-midnight hover:bg-navy-midnight hover:text-white transition-all cursor-pointer"
            >
            ...
            <input
              ...
              aria-label="Volumen"
              className="flex-1 accent-navy-midnight bg-navy-midnight/10 h-1.5 appearance-none cursor-pointer outline-none rounded-full"
            />
            <span className="w-10 shrink-0 text-right font-black text-xs text-navy-midnight">
