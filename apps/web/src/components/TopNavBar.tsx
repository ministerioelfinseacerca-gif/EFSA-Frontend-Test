        ...
        <nav className="hidden md:flex gap-8">
          {links.map((link, idx) => (
            <Link
              ...
              className={`font-body text-sm font-bold tracking-wider relative py-1 transition-colors duration-300 inline-block ${
                link.active
                  ? 'text-gold-highlight border-b border-gold-base/50'
                  : 'text-gold-base opacity-80 hover:text-gold-highlight hover:opacity-100'
              }`}
            >
              {link.label}
            </Link>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-surface-dark border-b border-gold-base/20 overflow-hidden"
          >
