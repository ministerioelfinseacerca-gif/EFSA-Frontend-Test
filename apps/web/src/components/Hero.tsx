...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      />
      {/* Increased overlay opacity to 50% for atmospheric mood, plus a bottom gradient for button legibility */}
      <div className="absolute inset-0 bg-black/50 z-10 hero-entrance" />
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 mt-12 hero-entrance hero-entrance-delay-3">
          ...
          <a
            href="/radio"
            className="group relative inline-flex items-center justify-center gap-2 border border-transparent text-[#D4AF37] bg-transparent font-headline text-sm font-bold uppercase tracking-widest px-6 py-3 rounded-full transition-all duration-500 hover:border-[#D4AF37]/30"
          >
            <div className="absolute inset-0 rounded-full bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out z-0" />
            <span className="relative z-10 w-2 h-2 rounded-full bg-efsa-red live-pulse" />
            <span className="relative z-10 transition-colors duration-500">Radio en Vivo</span>
          </a>
        </div>
      </div>
    </section>
